...
const ExperienceVoiceControl = () => {
  const navigate = useNavigate(); // Initialize navigate

  const handleCommand = useCallback((command) => {
    if (command.includes("certificate")) {
      window.open("https://drive.google.com/file/d/1Kgwx70km_Oin8R02GowU-yMPK-VQjsBN/view?usp=sharing", "_blank");
    } else if (command.includes("visit")) {
      window.open("https://crakcode.in", "_blank");
  ...
  }, [handleCommand]); // Now handleCommand is included safely
...
export default ExperienceVoiceControl;
